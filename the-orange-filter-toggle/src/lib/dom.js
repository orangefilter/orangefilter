...
export async function scanAndFilter(
  ...
  const reverseMode = settings.filterMode === 'lockhimup';
  ...
  if (settings.sensitivity !== 'pictures-only') {
    ...
    elements.forEach((el) => {
      ...
      const matches = containsKeywords(el, keywords);

      if (reverseMode) {
        // Lock Him Up: hide elements that DON'T match keywords
        if (!matches) {
          hideElement(el, 'Container does not match keywords (reverse mode)');
        }
      } else {
        // Lock Her Up: hide elements that DO match keywords (default)
        if (matches) {
  ...
  images.forEach((img) => {
    ...
    const contextMatches = keywords.some((k) =>
      context.toLowerCase().includes(k.toLowerCase())
    );

    if (reverseMode) {
      // Lock Him Up: hide images that DON'T match keywords
      if (!contextMatches && context.trim().length > 0) {
        hideElement(
          img,
          'Image context does not match keywords (reverse mode)'
        );
      } else if (
        !contextMatches &&
        settings.aiMode &&
      ...
      // Lock Her Up: hide images that DO match keywords (default) — no AI in this mode
      if (contextMatches) {
  ...
  const processNext = async () => {
    ...
    try {
      ...
      if (response && response.success) {
        ...
        if (reverseMode) {
          // Lock Him Up: hide images NOT classified as Orange (i.e. Safe images)
          if (!response.isBlocked && response.confidence < 0.65) {
            hideElement(
              img,
              `AI: not orange content (${confidencePct}%) - hidden in reverse mode`
            );
          }
          // Orange images are kept visible in reverse mode
        } else {
          // Lock Her Up: default behavior
          if (response.isBlocked) {
            hideElement(
              img,
              `AI detected ${response.layer} (${confidencePct}%)`
            );
