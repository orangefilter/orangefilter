...
const modeLockHerUp = document.getElementById('modeLockHerUp');
const modeLockHimUp = document.getElementById('modeLockHimUp');
const modeToggleContainer = document.getElementById('modeToggle');
const modeDesc = document.getElementById('modeDesc');

function updateModeUI(mode) {
  if (mode === 'lockhimup') {
    modeLockHerUp.classList.remove('active');
    modeLockHimUp.classList.add('active', 'active-him');
    modeToggleContainer.classList.add('him-mode');
    modeDesc.textContent = 'Showing ONLY orange content';
  } else {
    modeLockHimUp.classList.remove('active', 'active-him');
    modeLockHerUp.classList.add('active');
    modeToggleContainer.classList.remove('him-mode');
    modeDesc.textContent = 'Filtering out orange content';
  }
}
...
async function init() {
  ...
  updateModeUI(data.settings.filterMode || 'lockherup');
  ...
  modeLockHerUp.addEventListener('click', async () => {
    data.settings.filterMode = 'lockherup';
    updateModeUI('lockherup');
    await setStorage(data);
  });

  modeLockHimUp.addEventListener('click', async () => {
    data.settings.filterMode = 'lockhimup';
    updateModeUI('lockhimup');
    await setStorage(data);
  });
